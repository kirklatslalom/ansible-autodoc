...
class Parser:
    ...
    def _populate_doc_data(self):
        ...
        """
        for annotaion in self.config.get_annotations_names(
            special=True, automatic=True
        ):
    ...
    def _get_annotation(
        self, name, role="all", return_keys=False, return_item=None, return_multi=False
    ):
    ...
    def allow_multiple(self, name):
        if name in self.config.annotations:
            if (
                "allow_multiple" in self.config.annotations[name].keys()
    ...
    def about(self, l="md"):
        if l == "md":
            return (
                ...
                + self.config.autodoc_name
                + "]("
                + self.config.autodoc_url
                + ")"
            )
