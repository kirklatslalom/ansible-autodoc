...
class Generator:
    ...
    def _write_doc(self):
        ...
        for file in self.template_files:
            doc_file = (
                self.config.get_output_dir() + "/" + file[: -len(self.extension) - 1]
            )
        ...
        for file in self.template_files:
            ...
            self.log.trace(
                "[GENERATOR] Writing doc output to: "
                + doc_file
                + " from: "
                + source_file
            )
                    ...
                    if data is not None:
                        try:
                            data = (
                                ...
                                .from_string(data)
                                .render(self._parser.get_data(), r=self._parser)
                            )
                            ...
                            if not self.config.dry_run:
                                ...
                                self.log.info(
                                    "[GENERATOR][DRY] Writing to: " + doc_file
                                )
                        except jinja2.exceptions.UndefinedError as e:
                            self.log.error(
                                "Jinja2 templating error: <"
                                + str(e)
                                + '> when loading file: "'
                                + file
